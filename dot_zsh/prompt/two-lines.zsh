# This two line prompt configuration can be found courtesy of romkatv (Roman Perepelitsa)
...
function set-prompt() {
  ...
  # ~/foo/bar   master                  {vcs_info}
  # ==> %
  #
  # Top left:      Blue current directory follow by green git branch
  # Top right:     Green Git branch.
  # Bottom left:   '#' if root, '%' if not; green on success, red on error.
  # Bottom right:

  # TODO add clean/dirty indicator next to branch
  local top_left="%F{blue}%3~%f    %F{green}${git_branch}%f"
  local top_right="${vcs_info_msg_0_}"
  local bottom_left="%F{red}=>%f%# "
  # TODO set this to the current node info/ruby info/etc
  local bottom_right=''

  local REPLY
  fill-line "$top_left" "$top_right"

  # Print a blank line between prompts
  print
  PROMPT=$REPLY$'\n'$bottom_left
